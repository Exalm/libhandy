    ...
    if (priv->child_transition.is_cancelled) {
      if (priv->last_visible_child != NULL) {
        if (hdy_leaflet_get_fold (self) == HDY_FOLD_FOLDED) {
          ...
          gtk_widget_set_child_visible (priv->visible_child->widget, TRUE);
        }
 ...
 *
 * Sets whether or not @self allows swiping to the child on the left.
 *
 * Since: 0.0.12
 */
void
 ...
 * @self: a #HdyLeaflet
 *
 * Returns whether the #HdyLeaflet allows swiping to the child
 * on the left.
 *
 ...
 *
 * Sets whether or not @self allows swiping to the child on the right.
 *
 * Since: 0.0.12
 */
void
  ...
  if (priv->child_transition.tick_id > 0) {
    ...
    if (((direction < 0 && priv->child_transition.can_swipe_back) ||
        (direction > 0 && priv->child_transition.can_swipe_forward) ||
         automatic) && priv->fold == HDY_FOLD_FOLDED)
...
hdy_leaflet_class_init (HdyLeafletClass *klass)
{
  ...
  props[PROP_CAN_SWIPE_BACK] =
      g_param_spec_boolean ("can-swipe-back",
                            _("Can swipe back"),
                            _("Whether or not swipe gesture can be used to switch to the child on the left"),
                            FALSE,
                            G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
  ...
  props[PROP_CAN_SWIPE_FORWARD] =
      g_param_spec_boolean ("can-swipe-forward",
                            _("Can swipe forward"),
                            _("Whether or not swipe gesture can be used to switch to the child on the right"),
                            FALSE,
                            G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
