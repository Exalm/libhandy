...
struct _HdyDemoWindow
{
  ...
  HdyDrawer *drawer;
  GtkButton *drawer_anchor_button;
                                 ...
                                 HdyDemoWindow *self)
{
  ...
  gtk_orientable_set_orientation (GTK_ORIENTABLE (self->paginator), hdy_combo_row_get_selected_index (row));
}
...
hdy_demo_window_class_init (HdyDemoWindowClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, HdyDemoWindow, drawer_anchor_button);
...
hdy_demo_window_init (HdyDemoWindow *self)
{
  ...
  hdy_combo_row_set_selected_index (self->paginator_indicator_style_row, HDY_PAGINATOR_INDICATOR_STYLE_LINES);

  hdy_drawer_add_anchor (self->drawer, GTK_WIDGET (self->drawer_anchor_button));
